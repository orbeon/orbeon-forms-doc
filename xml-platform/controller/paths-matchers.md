...
_NOTE: This is independent from page or service authorization._
...
_NOTE: Navigation to pages that use matchers but that do not provide an internal XML submission or_ `<setvalue>` _elements will cause the requested path to have its literal value, in the example above_ `/user/([^/]+)/blog/([^/]+)`_. It is not advised to perform navigation this way._
