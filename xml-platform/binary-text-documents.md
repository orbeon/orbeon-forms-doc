...
In Orbeon Forms [XPL and pipelines](http://wiki.orbeon.com/forms/doc/developer-guide/xml-pipeline-language-xpl) only deal with XML documents. This means that between processor outputs and processor inputs in a pipeline, only pure XML infosets circulate. There is however often a need to handle non-XML data in pipelines, in particular:
...
_NOTE: For the curious, the Base64 encoding is documented in_ [_RFC 2045_](http://www.ietf.org/rfc/rfc2045.txt)_. This encoding represents binary data by mapping it to a set of 64 ASCII characters._

Such documents are not meant to be read by users, in the same way that regular binary files are not meant to be examined by users. Binary documents are generated by Orbeon Forms processors, like the [URL generator](processors/url-generator.md) and [converters](http://wiki.orbeon.com/forms/doc/developer-guide/processors-converters). They are consumed by processors like the [HTTP serializer](http://wiki.orbeon.com/forms/doc/developer-guide/processors-http-serializer), the [Email processor](http://wiki.orbeon.com/forms/doc/developer-guide/processors-email), and [converters](http://wiki.orbeon.com/forms/doc/developer-guide/processors-converters).
...
Unlike binary documents, text documents can easily be examined by users. They can also be easily manipulated by languages such as [XSLT](http://wiki.orbeon.com/forms/doc/developer-guide/processors-xslt-jaxp). Like binary documents, they are generated by Orbeon Forms processors, like the [URL generator](processors/url-generator.md) and [converters](http://wiki.orbeon.com/forms/doc/developer-guide/processors-converters). They are consumed by processors like the [HTTP serializer](http://wiki.orbeon.com/forms/doc/developer-guide/processors-http-serializer), the [Email processor](http://wiki.orbeon.com/forms/doc/developer-guide/processors-email), and [converters](http://wiki.orbeon.com/forms/doc/developer-guide/processors-converters).

## Streaming

Processors can stream binary and text documents by issuing a number of short character SAX events. It is therefore possible to generate "infinitely" long binary and text documents with a constant amount of memory, assuming both the sender and the receiver of the document are able to perform streaming. This is the case for example of the [URL generator](processors/url-generator.md) and the [HTTP serializer](http://wiki.orbeon.com/forms/doc/developer-guide/processors-http-serializer).
