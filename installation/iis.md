...
Download the following files by clicking on each link, then clicking on the "Download raw file" button, with an arrow pointing down, which is on the right of the page:
...
- Move `user.jsp` in the directory `webapps\ROOT`, inside the Tomcat directory (for instance C:\Program Files\Apache Software Foundation\Tomcat 10.1). This page will show the value of the `Orbeon-Username` header.
