...
![Orbeon Demo: Examples of Formulas](../.gitbook/assets/examples-of-formulas.png)
...
_NOTE: This works when the expression is outside repeat repetitions. For expressions within the same repeat, `$name` returns the closest control._
...
_NOTE: With Orbeon Forms 3.8 and 3.9, headers cannot be reliably accessed after the form is initialized, so this function should be used for setting initial values on controls only. See the next scenario for a workaround._
...
_NOTE: Because the file name and file type are sent by the client's browser, they cannot be trusted. This should only be considered a first level of data validation, and further validation based on the content must be performed at a later time if needed. See also issue_ [_#1838_](https://github.com/orbeon/orbeon-forms/issues/1838)_._
...
_NOTE: See_ [_issue #4981_](https://github.com/orbeon/orbeon-forms/issues/4981) _for availability of a fix that impacts this example._
