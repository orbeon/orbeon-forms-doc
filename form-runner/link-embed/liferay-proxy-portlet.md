...
Form Runner can be deployed [directly](liferay-full-portlet.md) into Liferay. However in some cases, it is desirable to deploy Form Runner _separately_. The Form Runner proxy portlet provides this type of deployment, with the following benefits:
...
![Controls form through the Liferay proxy portlet](../../.gitbook/assets/liferay-proxy-controls-liferay7.png)

## Security considerations

{% hint style="warning" %}
Even though you can configure the proxy portlet to display a specific app and form, by default it only delegates to Form Runner. So it is especially important to secure Form Runner with the appropriate form permissions.
{% endhint %}

[\[SINCE Orbeon Forms 2024.1.3\]](../../release-notes/orbeon-forms-2024.1.3.md)

The proxy portlet now filters incoming paths and blocks access to disallowed pages as follows:
...
* **Form Runner Page**: New Page
* **Form Runner app name**: `acme`
* **Form Runner form name**: `sales`
...
{% hint style="warning" %}
Form Runner can still be configured to navigate to other pages using processes and actions. If you create custom processes or actions that direct users to these pages, it is strongly recommended to enable form access permissions.
{% endhint %}
...
1.  Open the portlet preferences page
    ...
    ![](../../.gitbook/assets/liferay-proxy-open-preferences.png)
2.  Configure the preferences

    ![](../../.gitbook/assets/liferay-proxy-preferences.png)
...
Portal URL parameters specified with the `keep-parameters` portlet init parameter are read from the portal URL and re-appended to action, render and resource URLs produced by the proxy portlet:
...
Prior to Orbeon Forms 4.9, you need the following configuration in your [properties](../../configuration/properties/):
...
See the Performance Tuning section of the [Full Portlet Guide](liferay-full-portlet.md#performance-tuning).
...
* [Organizations](../access-control/organization.md)
