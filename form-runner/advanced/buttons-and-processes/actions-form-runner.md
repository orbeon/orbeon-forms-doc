...
For instance, you can have an `approve` process that set the current workflow stage with `set-workflow-stage(name = "approved")`. You can also use a [value templates](/xforms/core/attribute-value-templates.md) to dynamically determine the name of the workflow stage, as in `set-workflow-stage(name = "{if (//expense > 10000) then 'review-needed' else 'approved'}")`.
