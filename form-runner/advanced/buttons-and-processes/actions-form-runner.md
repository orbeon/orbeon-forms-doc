...
_NOTE: The `confirm` action is not synchronous, so the process must be suspended right after or the process will continue before the dialog is shown to the user._
...
* [`embedForm()` API](../../link-embed/javascript-api.md#embedform)
* [Adding and removing process callback functions](../../api/other/form-runner-javascript-api.md#adding-and-removing-process-callback-functions)
