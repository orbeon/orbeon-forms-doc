...
The `orbeon_i_current` and `orbeon_i_control_text` tables may contains references, via the `data_id` column, to entries in `orbeon_form_data`. The content of these tables is recreated when a [full database reindex](/form-runner/features/home-page.md#reindexing) is performed.
