...
Orbeon Forms supports storing data in [Amazon Simple Storage Service (S3)](https://aws.amazon.com/s3/) in several ways:
...
- [SINCE Orbeon Forms 2026.1] The [`send-s3` action](/form-runner/advanced/buttons-and-processes/actions-form-runner-send-s3.md) stores form data as XML directly into an S3 bucket.
- [Data attachments can be stored into an S3 bucket instead of the database](/configuration/properties/persistence.md#storing-attachments-in-the-filesystem-or-on-s3). This is done by using an S3 attachment provider (i.e. setting the `oxf.fr.persistence.provider.*.*.*.attachments` property to `s3`).
...
- `s3-path`: Defines the XPath expression used to determine the storage path for attachments. The path is relative to the root of the S3 bucket. It is evaluated against the XML data of the form. If the XPath expression evaluates to an empty string, attachments are stored at the root of the S3 bucket. This expression has the same constraints as [the PDF, TIFF and XML attachments filenames](https://doc.orbeon.com/configuration/properties/form-runner/form-runner-detail-page/form-runner-email#attachment-properties). The object key is this path followed by the attachment filename. [SINCE Orbeon Forms 2025.1.3] When [several attachments](/form-runner/advanced/buttons-and-processes/actions-form-runner-email.md#multiple-pdf-attachments) of the same email have the same filename, a number is added before the extension of each of them so that they don't overwrite each other, for example `form-1.pdf`, `form-2.pdf`, `form-3.pdf`. Attachments whose filename is unique within the email keep their name. To control the names instead, use the `oxf.fr.email.pdf.filename` property, for example with `fr:pdf-template-name()`.
...
- `[s3-property]`: One of the following properties:
    ...
    - `force-path-style`: When `true`, forces path-style access for S3 requests (default: `false`). [SINCE Orbeon Forms 2026.1, 2025.1.1, 2024.1.5]
...
- [Form Runner `send-s3` action](/form-runner/advanced/buttons-and-processes/actions-form-runner-send-s3.md)
