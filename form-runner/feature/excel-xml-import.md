...
You start the import from the Form Runner Import page, accessible from the [Summary page](summary-page.md) when the "Import" button is configured, or directly with the path:
...
_NOTE: This step only applies to the `excel-headings` format._
...
_NOTE: This step only applies to the `excel-named-ranges` and `xml-form-structure-and-data` formats._
...
For more about the format, see [Excel and XML Export](excel-xml-export.md).
...
_NOTE: This property only applies to the `excel-headings` format._
...
_NOTE: This property only applies to the `excel-named-ranges` and `xml-form-structure-and-data` formats._
