...
This page describes how to implement a custom persistence implementation (also known as a persistence _provider_) for Form Runner and Form Builder.
...
* `HEAD`: retrieve a resource's metadata
  ...
  * the persistence proxy _will_ issue `HEAD` requests, in the following cases, so the implementation must support it if the behavior is to be correct
...
Orbeon Forms supports the concept of [autosave drafts](../../persistence/autosave.md). With the built-in relational provider, a draft is marked specially in database tables with a `draft` column. A custom persistence provider that supports drafts must make that distinction in its storage mechanism.
...
Orbeon Forms supports the concept of [revision history](../../feature/revision-history.md).
...
The [Form Metadata API](forms-metadata.md) is used to retrieve the list of _published_ forms as well as associated metadata, including published versions, form titles, and permissions. There are some internal uses of this API, therefore a persistence provider should implement it.
...
* the [Form Runner Summary page](../../feature/summary-page.md)
...
* a form's Edit or New page, when looking for data associated with:
  * [Singleton form](../../advanced/singleton-form.md)
  * [Autosave drafts](../../persistence/autosave.md)
* the [Zip Export API](export-zip.md)
* the Purge API
...
* the [Revision history feature](../../feature/revision-history.md)
* the [Zip Export API](export-zip.md)
* the Purge API
...
* [Caching](caching.md)
* [Relational](../../persistence/relational-db.md)
