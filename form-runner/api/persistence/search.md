...
|Attribute|Required|Function|Comment|
|---|---|---|---|
|`path`|Yes|path to the XML element or attribute storing the data in XForms, relative to the root element|see NOTE 1|
|`control`|Yes|control type|see NOTE 2|
|text value|No|search string| |
|`match`|No|match type|future, see NOTE 3|
|`type`|No|datatype associated with the field in Form Builder|informative only|
|`search-field`|No| |informative only|
|`summary-field`|No| |informative only|
|`name`|No|control name as entered in Form Builder|informative only|
|`label`|No|control label|informative only|
...
- [SINCE Orbeon Forms 2020.1]
    - This is used to determine the type of match in the database:
        - for `input` and `textarea`, infer a substring match
        - for `select`, infer a space-separated exact token match
        - for others, infer an exact match 
...
- *NOTE: The Orbeon Forms relational persistence layer doesn't honor this correctly as of Orbeon Forms 2019.2. The eXist persistence layer does.*
