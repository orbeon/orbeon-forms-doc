...
The `<created-gte>` and `<created-lt>` elements are used to search by creation date:

- `created-gte`: minimum creation date/time in ISO format (inclusive)
- `created-lt`: maximum creation date/time in ISO format (exclusive)

The `<last-modified-gte>` and `<last-modified-lt>` elements are used to search by last modification date.

- `last-modified-gte`: minimum last modification date/time in ISO format (inclusive)
- `last-modified-lt`: maximum last modification date/time in ISO format (exclusive)

The `<created-by>` and `<last-modified-by>` elements are used to search by user who created or last modified the data. The `<workflow-stage>` element is used to search by workflow stage.
...
<search>
  <query/>
  <created-gte>2023-12-01T00:00:00+01:00</created-gte>
  <created-lt>2024-01-01T00:00:00+01:00</created-lt>
  <created-by>user-a</created-by>
  <last-modified-gte>2024-01-01T00:00:00+01:00</last-modified-gte>
  <last-modified-lt>2024-02-01T00:00:00+01:00</last-modified-lt>
  <last-modified-by>user-b</last-modified-by>
  <workflow-stage>initial-stage</workflow-stage>
  <page-size>10</page-size>
  <page-number>1</page-number>
</search>
```

All those elements are optional and can be left empty or missing.

## Sorting

[SINCE Orbeon Forms 2024.1]

Documents can be sorted by specifying `<order-by-column>` and `<order-by-direction>` elements in the query. Example:

```xml
<search>
  <query/>
  <order-by-column>details/title</order-by-column>
  <order-by-direction>asc</order-by-direction>
  <page-size>10</page-size>
  <page-number>1</page-number>
</search>
```

The `<order-by-column>` element contains the path of the control to sort by or one of the following special values:

- `created`: data creation date/time
- `created-by`: username of the user who created the data
- `last-modified`: data last modification date/time
...
The `<order-by-direction>` element contains the direction of the sort, either `asc` (ascending) or `desc` (descending).

Both of those elements are optional. The default sort order is by last modification date/time, descending.
