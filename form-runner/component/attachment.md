...
If an [`upload-mediatypes` custom constraint](../../contributors/extension-validation.md#xxfupload-mediatypes) is present and not blank, it is applied to the enclosed upload control.
...
In all cases, the mediatype provided by the browser cannot be trusted and is indicative only. You can use the [file scan API](../api/other/file-scan-api.md) to reject files that do not satisfy the indicated mediatype.
