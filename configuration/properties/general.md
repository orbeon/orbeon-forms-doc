...
For security reasons, Orbeon Forms disables XML _external_ entities. Other entities are enabled, but subject to a limit. This limit is set to 100,000 entity expansions.
...
You _don't need_ to set this property if:
...
With 4.6.x and earlier, you _don't need_ to set this property if:
...
* [Orbeon Forms version encryption](../advanced/javascript-css-assets.md#oxf.xforms.resources.encode-version) for cached assets URLs
...
* [`oxf.crypto.password`](general.md#oxf.crypto.password)
...
This property can easily be commented out for deployment, or placed in `properties-local-dev.xml` (see also [Run Modes](../advanced/run-modes.md)).
