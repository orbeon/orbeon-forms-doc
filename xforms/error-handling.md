...
* **Other static XForms errors**: For example, duplicate `id` attributes on elements.
...
_NOTE: Until Orbeon Forms 4.8, these errors were logged at_ `warning` _level._
...
The following property allows you to enable or disable showing recoverable server errors to the user via the error dialog, and to determine a maximum number of errors to show the user. This is intended as a _development_ feature, not a production feature.
...
_NOTE: From Orbeon Forms 4.0 to 4.8, the default for_ `dev` _was_ `10`_. Because few people use the_ `dev` _mode and because having a different behavior in development vs. production can be confusing, the default has been changed to_ `0` _in both cases._
