...
_NOTE: Since_ `POST` _and_ `PUT` _are not meant to be idempotent methods, you should use_ `xxf:cache="true"` _with these methods carefully, typically for calls to services you know are idempotent. Otherwise, incorrect or stale data might be returned by the submission._
...
_SECURITY NOTE: When using_ `xxf:cache="true"`_, be sure that the data contained in the instance does not contain information that could be inadvertently shared with other XForms documents. It is recommended to use it to load localized resources or similar types of data._
