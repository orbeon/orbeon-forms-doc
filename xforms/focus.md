...
The XForms [`<xf:setfocus>`](http://www.w3.org/TR/xforms11/#action-setfocus) action allows the form author to change the focus to a particular control. It is similar to the HTML `focus()` function, with handling of grouping controls as a nice bonus feature.
...
* resolve the target `control` attribute following [Resolving ID References in XForms](http://www.w3.org/TR/xforms11/#idref-resolve) &#x20;
* dispatch the [`xforms-focus`](http://www.w3.org/TR/xforms11/#evt-focus) event to that control
* the control then searches for the actual control to focus, if any, following the algorithm below

Orbeon Forms implements the following XForms 1.1-compatible behavior when receiving [`xforms-focus`](http://www.w3.org/TR/xforms11/#evt-focus):
...
_NOTE: the_ `xforms-focus` _event does not bubble._
