...
_NOTE: Prior to XForms 1.1, the_ `at` _attribute was mandatory. Since XForms 1.1, it is optional, and if omitted the_ `ref` _attribute specifies all the nodes to remove._

## Inserting iterations with the insert action

_See also the_ [_Insert a new item into a repeat_](http://wiki.orbeon.com/forms/how-to/logic/repeat-insert-position) _how-to guide._
...
Orbeon Forms supports the `xxf:default` extension attribute on `xf:bind` to specify [dynamic initial values](../binds.md#dynamic-initial-values).
...
* [Dynamic initial values](../binds.md#dynamic-initial-values)
